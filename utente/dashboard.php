<?php
    ...
    try{
        ...
        $stmt = $pdo->prepare("SELECT *
                               ...
                               LIMIT 1");
        $stmt->execute([':id_utente' => $id_utente]);
        $abbonamento = $stmt->fetch(PDO::FETCH_ASSOC);
...
<div class="container">
    ...
    <div class="dashboard-grid">
        <div class="col-left">
            ...
            <div class="card-box card-highlight mb-20">
                <div class="card-icon">💳</div>
                <h3>Stato Abbonamento</h3>
                <?php if($abbonamento){ 
                    $oggi = date('Y-m-d');
                    $is_scaduto = ($abbonamento['data_scadenza'] < $oggi) || ($abbonamento['stato'] === 'Scaduto');
                    $classe_stato = $is_scaduto ? 'error-msg' : 'success-msg';
                    $testo_stato = $is_scaduto ? 'SCADUTO' : strtoupper($abbonamento['stato']);
                ?>
                    <p>Tipo: <strong><?=htmlspecialchars($abbonamento['tipo_abbonamento'])?></strong></p>
                    <p>Scadenza: <strong><?=date('d/m/Y', strtotime($abbonamento['data_scadenza']))?></strong></p>
                    
                    <div class="<?=$classe_stato?> mt-20">
                        STATO: <?=$testo_stato?>
                    </div>
                <?php }else{ ?>
                    <p>Nessun abbonamento registrato.</p>
                    <div class="error-msg mt-20">NON ATTIVO</div>
                <?php } ?>
            </div>
            <div class="card-box">
        ...
        <div class="col-right">
            ...
            <div class="card-box">
                ...
                <a href="progressi.php" class="btn btn-outline">Vedi Storico</a>
            </div>
        </div>
    </div>
</div>
